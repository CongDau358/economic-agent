...
Tất cả admin endpoints yêu cầu `X-API-Key` header.
Prefix: `/admin`

| Method | Path | Mô tả |
|--------|------|-------|
| `DELETE` | `/admin/cache` | Xóa toàn bộ cache |
| `DELETE` | `/admin/jobs` | Xóa job history (done + failed) |
| `GET` | `/admin/stats` | Thống kê vector store, jobs, cache, metrics |
| `POST` | `/admin/reindex` | Reindex documents từ processed files |
| `DELETE` | `/admin/collection` | Xóa documents của 1 công ty |
| `GET` | `/admin/documents` | Liệt kê documents theo company/sector |
| `POST` | `/admin/metrics/reset` | Reset usage metrics |
...
```json
{
  "vector_store": {
    "total_documents": 142,
    "companies": { "Vinamilk": 24, "VCB": 18 },
    "collection": "economic_docs"
  },
  "jobs":    { "total": 5, "done": 4, "failed": 1 },
  "cache":   { "total_keys": 12, "live_keys": 10 },
  "metrics": { "total_calls": 87, "total_errors": 2 }
}
```
...
```json
// Request body
{ "company": "Vinamilk", "confirm": true }

// Response
{ "company": "Vinamilk", "deleted": 24, "message": "Đã xóa 24 documents..." }
```

⚠ Không thể hoàn tác. Phải truyền `confirm: true`.

### GET /admin/documents

Query params: `company`, `sector`, `limit` (1-100, default 20)

```json
{
  "documents": [
    { "id": "abc123", "company": "Vinamilk", "source": "Q3_report.pdf", "ingested_at": "2024-10-01T..." }
  ],
  "count": 1
}
```